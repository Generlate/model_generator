...
if __name__ == "__main__":
    ...
    TRAINING_INPUT: torch.Tensor = (
        utils.data_formatter.TRAINING_COMBINED_TENSOR
    )
    TESTING_INPUT: torch.Tensor = utils.data_formatter.TESTING_COMBINED_TENSOR

    # Set number of epochs. I found three to give the lowest loss score.
    NUMBER_OF_EPOCHS: int = 3
    TESTING_DATA_ITERATOR: Iterator[torch.Tensor] = iter(TESTING_INPUT)
    ...
    for EPOCH in range(NUMBER_OF_EPOCHS):
        ...
        OUTPUT: torch.Tensor = model(TRAINING_INPUT)

        # Compare the generated coordinates against the test coordinates.
        LOSS: torch.Tensor = CRITERION(OUTPUT, TESTING_DATA_TENSOR)
    ...
    # Format to .off.
    FORMATTED_ARRAY: str = "OFF\n8 6 0\n"
    for i, VALUE in enumerate(ARRAY):
        FORMATTED_ARRAY += str(VALUE * 22) + " "
        if (i + 1) % 3 == 0:
            FORMATTED_ARRAY += "\n"

    ADDITIONAL_STRING: str = """4 0 1 2 3
    4 1 5 6 2
    4 5 4 7 6
    4 4 0 3 7
    4 3 2 6 7
    ...
    file_path: str = "./generated_boxes/generated_box.off"
